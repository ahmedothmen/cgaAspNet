    ...
    public class VehicleWreckController : Controller
    {
        ...
        public ActionResult Index(string option, string search, int? pageNumber)
        {
            ...
            if (option == "numchassis")
            {
                ...
                return View(vm.Where(x =>x.numchassis == search || search == null).ToList().ToPagedList(pageNumber ?? 1, 4));
            }
            else if (option == "couleur")
            {
                return View(vm.Where(x => x.couleur == search || search == null).ToList().ToPagedList(pageNumber ?? 1, 4));
            }
            else if (option == "description")
            {
                return View(vm.Where(x => x.description == search || search == null).ToList().ToPagedList(pageNumber ?? 1, 4));
            }
            else
            {
                return View(vm.Where(x => x.modele == search || search == null).ToList().ToPagedList(pageNumber ?? 1, 4));
            }
            return View(cc.vehicleswreck.OrderByDescending(x => x.id).ToPagedList(pageNumber ?? 1, 4));
        }
